...
use renvo_cpu_pic16::{Pic16Cpu, Pic16Register};
use renvo_devices::{GpioHandle, Pic16Peripherals, Pic16PeripheralsHandle, SignalHub};
use renvo_image::ProgramWordImage;
use renvo_signals::Logic;
use renvo_trace::{TraceDigest, TraceSink};
...
pub enum Pic16MachineError {
    ...
    #[error(transparent)]
    Map(#[from] renvo_bus::MapError),
    /// Signal construction or lookup failed.
    #[error(transparent)]
    Signal(#[from] renvo_signals::SignalError),
    /// CPU loading or execution failed.
    #[error(transparent)]
    Cpu(#[from] renvo_core::CpuFault),
    /// Package pin drive failed.
    #[error(transparent)]
    Device(#[from] renvo_bus::DeviceError),
    /// Trace output failed.
    #[error(transparent)]
    Trace(#[from] renvo_trace::TraceError),
}
...
impl Pic16McuMachine {
    ...
    pub fn debug_snapshot(&self) -> renvo_core::CpuSnapshot {
        self.cpu.snapshot()
    }
    ...
    pub fn set_pin(&self, pin: u8, value: Logic) -> Result<(), Pic16MachineError> {
        let (port, local_pin) = match pin {
            ...
            32..=35 => (4, pin - 32),
            _ => {
                return Err(renvo_bus::DeviceError::new(format!(
...
mod tests {
    use super::*;
    use renvo_image::{ProgramWordEndianness, ProgramWordSegment};

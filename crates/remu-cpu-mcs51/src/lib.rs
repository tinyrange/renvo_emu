...
pub struct Mcs51Cpu {
    ...
    pc: u16,
    sfr_page: u8,
    // Six modeled interrupt sources (Timer0, UART0, Timer2, Timer3, Timer4,
    // and Timer5) plus the reserved UART1 pair, each with low/high inputs.
    interrupts: [bool; 14],
...
impl Mcs51Cpu {
    ...
    fn pending_interrupt(&self) -> Option<(usize, bool)> {
        for high in [true, false] {
            ...
            // Preserve the device's documented vector priority order while
            // keeping each source's low/high input adjacent in the array.

...
impl Mcs51Cpu {
    ...
    fn pending_interrupt(&self) -> Option<(usize, bool)> {
        for high in [true, false] {
            ...
            // Preserve the classic six-line mapping (0/3, 1/4, 2/5) and
            // place Timer1 on the spare line pair (6/7).
            let lines = if high { [3, 4, 5, 7] } else { [0, 1, 2, 6] };
            if let Some(line) = lines.into_iter().find(|line| self.interrupts[*line]) {
    ...
    fn enter_interrupt(&mut self, line: usize, high: bool) {
        let vector = match line {
            ...
            2 | 5 => 0x002b,
            6 | 7 => 0x001b,
            _ => unreachable!("interrupt line is one of the eight MCS-51 inputs"),
        };
        self.push_pc();

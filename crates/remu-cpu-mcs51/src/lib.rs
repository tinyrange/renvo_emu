...
impl Mcs51Cpu {
    ...
    fn pending_interrupt(&self) -> Option<(usize, bool)> {
        for high in [true, false] {
            ...
            let extended_line = if high { 7 } else { 6 };
            if self.interrupts[extended_line] {
                return Some((extended_line, high));
            }
        }
        None
    }
    ...
    fn enter_interrupt(&mut self, line: usize, high: bool) {
        const VECTORS: [u16; 8] = [
            0x000b, 0x0023, 0x002b, 0x000b, 0x0023, 0x002b, 0x009b, 0x009b,
        ];
        self.push_pc();

...
impl Mcs51Cpu {
    ...
    fn pending_interrupt(&self) -> Option<(usize, bool)> {
        for high in [true, false] {
            ...
            let port_match_line = if high { 7 } else { 6 };
            if self.interrupts[port_match_line] {
                return Some((port_match_line, high));
            }
        }
        None
    }
    ...
    fn enter_interrupt(&mut self, line: usize, high: bool) {
        const VECTORS: [u16; 8] = [
            0x000b, 0x0023, 0x002b, 0x000b, 0x0023, 0x002b, 0x0043, 0x0043,
        ];
        self.push_pc();
        self.priority_stack.push(self.active_priority);
        self.active_priority = Some(high);
        self.pc = VECTORS[line];
        self.waiting = false;
    }
}

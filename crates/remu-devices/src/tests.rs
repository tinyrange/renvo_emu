...
fn rp2040_watchdog_counts_down_with_e1_divide_by_two() {
    ...
    assert_eq!(
        watchdog_read(&mut device, Rp2040WatchdogRegister::Ctrl, 1) & 0x00ff_ffff,
        2
    );
    ...
    assert_eq!(
        watchdog_read(&mut device, Rp2040WatchdogRegister::Reason, 3),
        1
    );
}
...
fn rp2040_watchdog_force_trigger_and_scratch_reset_semantics_are_deterministic() {
    ...
    watchdog_write(&mut device, Rp2040WatchdogRegister::Tick, 0, 0);
    watchdog_write(&mut device, Rp2040WatchdogRegister::Load, 2, 0);
    watchdog_write(&mut device, Rp2040WatchdogRegister::Ctrl, 1 << 30, 0);
    assert!(!handle.take_reset(SimTime::from_ticks(8)));
    watchdog_write(&mut device, Rp2040WatchdogRegister::Tick, 1 << 9, 8);
    assert!(handle.take_reset(SimTime::from_ticks(9)));

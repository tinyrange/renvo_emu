...
mod tests {
    ...
    fn write_block(device: &mut EspC6Hmac, message: &[u8]) {
    ...
    fn read_result(device: &mut EspC6Hmac) -> [u8; RESULT_BYTES] {
    ...
    fn one_block_hmac_matches_known_sha256_vector() {
        let mut device = EspC6Hmac::new("hmac");
        device

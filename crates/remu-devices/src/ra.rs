...
mod tests {
    ...
    fn gpt_and_sci_events_route_through_ielsr() {
        ...
        for (event, line) in [
            (RA4M1_EVENT_GPT5_OVERFLOW, 8),
            (RA4M1_EVENT_GPT6_OVERFLOW, 9),
            (RA4M1_EVENT_GPT7_OVERFLOW, 10),
        ] {
            icu.write(
                0x300 + line * 4,
                AccessWidth::Word,
            ...
            .unwrap();
            assert_eq!(handle.route_event(event), vec![line as u16]);
        }
        ...
        let (mut gpt16, gpt16_handle) = RaGpt::new_16("gpt5");
        gpt16
            .write(
                RaGptRegister::Gtpr.offset(),
                AccessWidth::Word,

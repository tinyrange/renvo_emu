...
/// Host-facing STM32 TIM15 timing and channel-one handle.
#[derive(Clone)]
pub struct Stm32Tim15Handle {
    state: Arc<Mutex<Tim15State>>,
    hub: SignalHub,
    interrupt_signal: SignalId,
    output_signal: SignalId,
}

impl Stm32Tim15Handle {
    /// Advances TIM15 and returns whether an enabled update/compare interrupt is pending.
    pub fn poll(&self, now: SimTime) -> bool {
        let (pending, output) = {
            let mut state = self.state.lock().expect("STM32 TIM15 lock poisoned");
            if state.cr1 & CR1_CEN != 0 {
                ...
                if elapsed != 0 {
                    ...
                    let compare_hit = state.ccer & CCER_CC1E != 0
            ...
            (
                ...
                Stm32Tim15Handle::channel_output(&state),
            )
        };
    ...
    /// Current TIM15 counter.
    pub fn counter(&self) -> u16 {
        self.state.lock().expect("STM32 TIM15 lock poisoned").cnt
    }

    /// Current functional channel-one output level.
    pub fn channel_one_output(&self) -> bool {
        let state = self.state.lock().expect("STM32 TIM15 lock poisoned");
        Self::channel_output(&state)
    }

    fn channel_output(state: &Tim15State) -> bool {
        if state.ccer & CCER_CC1E == 0 {
            return false;
        }
        let mode = (state.ccmr1 >> 4) & 0x7;
        let active = match mode {
            6 => u32::from(state.cnt < state.ccr1),
            7 => u32::from(state.cnt >= state.ccr1),
            _ => 0,
        } != 0;
        ...
        self.hub
            .set(
                ...
                SignalValue::from_u64(u64::from(pending), 1)
                    .expect("TIM15 interrupt signal width is valid"),
                at,
            )
            .expect("TIM15 interrupt signal is declared");
        self.hub
            .set(
                self.output_signal,
                SignalValue::from_u64(u64::from(output), 1)
                    .expect("TIM15 output signal width is valid"),
                at,
            )
            .expect("TIM15 output signal is declared");
    }
}

/// Functional STM32L432 TIM15 counter/update and channel-one PWM slice.
pub struct Stm32Tim15 {
    name: String,
    state: Arc<Mutex<Tim15State>>,
    handle: Stm32Tim15Handle,
}

impl Stm32Tim15 {
    /// Creates a disabled TIM15 with a maximal auto-reload value.
    pub fn new(
        name: impl Into<String>,
        hub: SignalHub,
    ) -> Result<(Self, Stm32Tim15Handle), remu_signals::SignalError> {
        let name = name.into();
        let interrupt_signal = hub.declare(
            format!("{name}.irq"),
            SignalValue::from_u64(0, 1)?,
            Some("TIM15 update/compare interrupt request".to_owned()),
        )?;
        let output_signal = hub.declare(
            format!("{name}.ch1"),
            SignalValue::from_u64(0, 1)?,
            Some("TIM15 channel-one functional output".to_owned()),
        )?;
        let state = Arc::new(Mutex::new(Tim15State {
            arr: u16::MAX,
            ..Tim15State::default()
        }));
        let handle = Stm32Tim15Handle {
            state: state.clone(),
            hub,
            interrupt_signal,
            output_signal,
...
impl Device for Stm32Tim15 {
    ...
    fn read(&mut self, offset: u64, width: AccessWidth, at: SimTime) -> Result<u64, DeviceError> {
        if width != AccessWidth::Word {
            return Err(DeviceError::new("STM32 TIM15 requires word accesses"));
        }
        if offset & 3 != 0 || offset >= 0x400 {
            return Err(DeviceError::new(format!(
                "STM32 TIM15 access at {offset:#x}"
            )));
        }
    ...
    fn reset(&mut self, _kind: ResetKind) {
        *self.state.lock().expect("STM32 TIM15 lock poisoned") = Tim15State {
            arr: u16::MAX,
            ..Tim15State::default()
        };
...
mod tests {
    ...
    #[test]
    fn tim15_update_compare_and_pwm_output_are_observable() {
        let hub = SignalHub::new();
        let (mut timer, handle) = Stm32Tim15::new("tim15", hub).unwrap();
        timer

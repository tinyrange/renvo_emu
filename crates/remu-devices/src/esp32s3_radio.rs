...
const WIFI_MAC_RX_BA_SEQUENCE_HIGH: u64 = 0x280;
const WIFI_MAC_RX_BA_BITMAP_LOW_HIGH: u64 = 0x284;
const WIFI_MAC_RX_BA_BITMAP_HIGH_HIGH: u64 = 0x288;
const WIFI_MAC_RX_BA_STRIDE: u64 = 0x24;
const WIFI_MAC_RX_BA_COUNT: usize = 8;
...
impl Esp32S3WifiMacHandle {
    fn rx_block_ack_slot(state: &Esp32S3WifiMacState, peer: &[u8; 6], tid: u8) -> Option<usize> {
        (0..WIFI_MAC_RX_BA_COUNT).find(|slot| {
            ...
            let control = state.registers[(WIFI_MAC_RX_BA_CONTROL_HIGH - distance) as usize / 4];
            let mac_low = state.registers[(WIFI_MAC_RX_BA_MAC_LOW_HIGH - distance) as usize / 4];
            let mac_high = state.registers[(WIFI_MAC_RX_BA_MAC_HIGH_HIGH - distance) as usize / 4];
            control & WIFI_MAC_RX_BA_ACTIVE == WIFI_MAC_RX_BA_ACTIVE
                && control & 0x0fff == WIFI_MAC_RX_BA_MODE
    ...
    pub fn validate_block_ack_sessions(&self) -> Result<(), String> {
        ...
        for slot in 0..WIFI_MAC_RX_BA_COUNT {
            let distance = slot as u64 * WIFI_MAC_RX_BA_STRIDE;
            let control = state.registers[(WIFI_MAC_RX_BA_CONTROL_HIGH - distance) as usize / 4];
            if control & WIFI_MAC_RX_BA_VALID != 0
                && (control & WIFI_MAC_RX_BA_ACTIVE != WIFI_MAC_RX_BA_ACTIVE
                    || control & 0x0fff != WIFI_MAC_RX_BA_MODE)
            {
                return Err(format!(
    ...
    pub fn record_block_ack_mpdu(&self, peer: &[u8; 6], tid: u8, sequence: u16) -> bool {
        ...
        let distance = slot as u64 * WIFI_MAC_RX_BA_STRIDE;
        let sequence_index = (WIFI_MAC_RX_BA_SEQUENCE_HIGH - distance) as usize / 4;
        let bitmap_low_index = (WIFI_MAC_RX_BA_BITMAP_LOW_HIGH - distance) as usize / 4;
        let bitmap_high_index = (WIFI_MAC_RX_BA_BITMAP_HIGH_HIGH - distance) as usize / 4;
    ...
    pub fn block_ack_bitmap(&self, peer: &[u8; 6], tid: u8, starting_sequence: u16) -> Option<u64> {
        ...
        let distance = slot as u64 * WIFI_MAC_RX_BA_STRIDE;
        let origin =
            state.registers[(WIFI_MAC_RX_BA_SEQUENCE_HIGH - distance) as usize / 4] as u16 & 0x0fff;
        let bitmap =
            u64::from(state.registers[(WIFI_MAC_RX_BA_BITMAP_LOW_HIGH - distance) as usize / 4])
                | (u64::from(
                    state.registers[(WIFI_MAC_RX_BA_BITMAP_HIGH_HIGH - distance) as usize / 4],
                ) << 32);

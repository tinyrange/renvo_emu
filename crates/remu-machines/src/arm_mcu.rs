...
use remu_devices::{
    ...
    FunctionalUart, GpioHandle, RA4M1_EVENT_GPT0_OVERFLOW, RA4M1_EVENT_SCI9_TXI, RaCrc,
    RaCrcHandle, RaGpt, RaGptHandle, RaIcu, RaIcuHandle, RaIoPort, RaPfs, RaSci, RaSciHandle,
...
impl ArmMcuMachine {
    ...
    pub fn new(target: TargetId) -> Result<Self, ArmMachineError> {
        ...
        let (gpio, uart, timer, eic, ra_icu, crc, watchdog) = match target {
            TargetId::Atsamd21e18 => {
    ...
    pub fn crc_value(&self) -> Option<u32> {
        self.crc.as_ref().map(RaCrcHandle::value)
    }

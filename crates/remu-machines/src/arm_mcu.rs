...
use remu_devices::{
    ...
    FunctionalUart, GpioHandle, RA4M1_EVENT_GPT0_OVERFLOW, RA4M1_EVENT_SCI9_TXI, RaDac,
    RaDacHandle, RaGpt, RaGptHandle, RaIcu, RaIcuHandle, RaIoPort, RaPfs, RaSci, RaSciHandle,
...
impl ArmMcuMachine {
    ...
    pub fn new(target: TargetId) -> Result<Self, ArmMachineError> {
        ...
        let (gpio, uart, timer, eic, ra_icu, dac, watchdog) = match target {
            TargetId::Atsamd21e18 => {
    ...
    pub fn dac_value(&self) -> Option<u16> {
        self.dac.as_ref().map(RaDacHandle::value)
    }
...
mod tests {
    ...
    #[test]
    fn ra4m1_maps_dac12_data_and_output_enable() {
        let mut machine = ArmMcuMachine::new(TargetId::R7fa4m1ab3cfm).unwrap();
        machine
            .bus

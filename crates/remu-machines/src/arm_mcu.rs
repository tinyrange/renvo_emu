...
use remu_devices::{
    ...
    FunctionalUart, GpioHandle, RA4M1_EVENT_GPT0_OVERFLOW, RA4M1_EVENT_GPT4_OVERFLOW,
...
const RA4M1_GPT4_BASE: u64 = 0x4007_8400;
...
pub struct ArmMcuMachine {
    ...
    timer: VendorTimer,
    ra_gpt4: Option<RaGptHandle>,
    ra_gpt4_irq_signal: Option<SignalId>,
...
impl ArmMcuMachine {
    ...
    pub fn new(target: TargetId) -> Result<Self, ArmMachineError> {
        ...
        let ra_gpt4_irq_signal = if target == TargetId::R7fa4m1ab3cfm {
            Some(signals.declare(
                "board.r7fa4m1ab3cfm.gpt4.irq",
                SignalValue::from_u64(0, 1)?,
                Some("functional GPT4 overflow request".to_owned()),
            )?)
        } else {
            None
        };
        ...
        let (gpio, uart, timer, ra_gpt4, eic, ra_icu, watchdog) = match target {
            TargetId::Atsamd21e18 => {
            ...
            TargetId::R7fa4m1ab3cfm => {
                ...
                let (gpt4_device, gpt4) = RaGpt::new_16("r7fa4m1ab3cfm.gpt4");
    ...
    fn map_ra4m1(
        ...
        bus.map_device("r7fa4m1ab3cfm.gpt4", RA4M1_GPT4_BASE, 0x100, Box::new(gpt4))?;
    ...
    pub fn run_with_stimuli(
        ...
        let reason = loop {
            ...
            if let (Some(gpt4), Some(icu), Some(signal)) =
                (&self.ra_gpt4, &self.ra_icu, self.ra_gpt4_irq_signal)
            {
                let gpt4_pending = gpt4.poll(self.now);
                interrupt_requested |= gpt4_pending;
                if gpt4_pending {
                    for line in icu.route_event(RA4M1_EVENT_GPT4_OVERFLOW) {
                        self.cpu
                            .set_interrupt(line, self.ppb.interrupt_enabled(line))?;
                    }
                }
                self.signals.set(
                    signal,
                    SignalValue::from_u64(u64::from(gpt4_pending), 1)?,
                    self.now,
                )?;
            }
            match self.target {
...
mod tests {
    ...
    fn ra4m1_uses_m4f_and_its_own_ioport_and_icu_map() {
        ...
        machine
            .bus
            .write(
                RA4M1_GPT4_BASE + RaGptRegister::Gtpr.offset(),
                AccessWidth::Word,
                3,
                SimTime::ZERO,
            )
        ...
        machine
            .bus
            .write(
                RA4M1_GPT4_BASE + RaGptRegister::Gtintad.offset(),
                AccessWidth::Word,
                1 << 6,
                SimTime::ZERO,
            )
        ...
        machine
            .bus
            .write(
                RA4M1_GPT4_BASE + RaGptRegister::Gtcr.offset(),
                AccessWidth::Word,
                1,
                SimTime::ZERO,
            )

...
use remu_devices::{
    ...
    Samd21UsartHandle, Samd21Wdt, Samd21WdtHandle, SignalHub, Stm32BasicTimer,
    Stm32BasicTimerHandle, Stm32Gpio, Stm32Timer, Stm32TimerHandle, Stm32Usart, Stm32UsartHandle,
...
impl ArmMcuMachine {
    ...
    pub fn new(target: TargetId) -> Result<Self, ArmMachineError> {
        ...
        let (gpio, uart, timer, stm32_tim6, eic, ra_icu, watchdog) = match target {
            TargetId::Atsamd21e18 => {
    ...
    pub fn run_with_stimuli(
        ...
        let reason = loop {
            ...
            let mut interrupt_requested = timer_pending || tim6_pending;

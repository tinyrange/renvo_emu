...
impl ArmMcuMachine {
    ...
    pub fn new(target: TargetId) -> Result<Self, ArmMachineError> {
        ...
        let (gpio, uart, timer, ra_gpt4, eic, ra_icu, watchdog) = match target {
            ...
            TargetId::R7fa4m1ab3cfm => {
                ...
                let (gpt4_device, gpt4) = RaGpt::new("r7fa4m1ab3cfm.gpt4");
    ...
    fn map_ra4m1(
        ...
        bus.map_device("r7fa4m1ab3cfm.gpt4", 0x4007_8400, 0x100, Box::new(gpt4))?;
...
mod tests {
    ...
    fn ra4m1_uses_m4f_and_its_own_ioport_and_icu_map() {
        ...
        machine
            .bus
            .write(0x4007_8464, AccessWidth::Word, 3, SimTime::ZERO)
            .unwrap();
        machine
            .bus
            .write(0x4007_8438, AccessWidth::Word, 1 << 6, SimTime::ZERO)
            .unwrap();
        machine
            .bus
            .write(0x4007_842c, AccessWidth::Word, 1, SimTime::ZERO)
            .unwrap();
        assert_eq!(
            machine
                .bus

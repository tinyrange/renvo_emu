...
use remu_devices::{
    ...
    Samd21UsartHandle, Samd21Wdt, Samd21WdtHandle, SignalHub, Stm32Gpio, Stm32Lptim2,
    Stm32Lptim2Handle, Stm32Timer, Stm32TimerHandle, Stm32Usart, Stm32UsartHandle, TimerHandle,
    UartHandle,
};
...
pub struct ArmMcuMachine {
    ...
    timer: VendorTimer,
    lptim2: Option<Stm32Lptim2Handle>,
...
impl ArmMcuMachine {
    ...
    pub fn new(target: TargetId) -> Result<Self, ArmMachineError> {
        ...
        let (gpio, uart, timer, lptim2, eic, ra_icu, watchdog) = match target {
            TargetId::Atsamd21e18 => {
    ...
    pub fn run_with_stimuli(
        ...
        let reason = loop {
            ...
            if self.target == TargetId::Stm32l432kc {
                self.cpu
                    .set_interrupt(66, lptim_pending && self.ppb.interrupt_enabled(66))?;
            }
            match self.target {

...
use remu_devices::{
    ...
    Samd21UsartHandle, Samd21Wdt, Samd21WdtHandle, SignalHub, Stm32Gpio, Stm32Sai1,
    Stm32Sai1Handle, Stm32Timer, Stm32TimerHandle, Stm32Usart, Stm32UsartHandle, TimerHandle,
    UartHandle,
};
...
impl ArmMcuMachine {
    ...
    pub fn new(target: TargetId) -> Result<Self, ArmMachineError> {
        ...
        let (gpio, uart, timer, sai1, eic, ra_icu, watchdog) = match target {
            TargetId::Atsamd21e18 => {
            ...
            TargetId::Stm32l432kc => {
                ...
                let (sai_device, sai1) = Stm32Sai1::new("stm32l432kc.sai1", signals.clone())?;
    ...
    pub fn run_with_stimuli(
        ...
        let reason = loop {
            ...
            let mut interrupt_requested = timer_pending || sai_pending;
...
mod tests {
    ...
    #[test]
    fn stm32l432_native_sai1_maps_block_registers_and_enables_sample_output() {
        let mut machine = ArmMcuMachine::new(TargetId::Stm32l432kc).unwrap();
        ...
        machine
            ...
            .unwrap();
        assert!(machine.sai1.as_ref().expect("STM32 SAI1 handle").enabled(0));
        assert_eq!(
            machine
                .sai1
                .as_ref()
                .expect("STM32 SAI1 handle")
                .take_tx(0)
                .unwrap(),
            vec![0x1234]
        );

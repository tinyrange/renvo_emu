...
use remu_devices::{
    ...
    Samd21UsartHandle, Samd21Wdt, Samd21WdtHandle, SignalHub, Stm32Gpio, Stm32Tim16,
    Stm32Tim16Handle, Stm32Timer, Stm32TimerHandle, Stm32Usart, Stm32UsartHandle, TimerHandle,
    UartHandle,
};
...
impl ArmMcuMachine {
    ...
    pub fn new(target: TargetId) -> Result<Self, ArmMachineError> {
        ...
        let (gpio, uart, timer, stm32_tim16, eic, ra_icu, watchdog) = match target {
            TargetId::Atsamd21e18 => {
            ...
            TargetId::Stm32l432kc => {
                ...
                let (tim16_device, stm32_tim16) =
                    Stm32Tim16::new("stm32l432kc.tim16", signals.clone())?;
    ...
    pub fn run_with_stimuli(
        ...
        let reason = loop {
            ...
            let mut interrupt_requested = timer_pending || tim16_pending;
            ...
            if self.stm32_tim16.is_some() {
                self.cpu
                    .set_interrupt(25, tim16_pending && self.ppb.interrupt_enabled(25))?;
            }
            match self.target {
...
mod tests {
    ...
    fn stm32l432_native_tim16_maps_counter_compare_and_pwm_registers() {
        ...
        machine
            .bus
            .write(base + 0x18, AccessWidth::Word, 6 << 4, SimTime::ZERO)
            .unwrap();
        machine
            .bus
            .write(base + 0x20, AccessWidth::Word, 1, SimTime::ZERO)
            .unwrap();
        machine
            .bus
            .write(base + 0x0c, AccessWidth::Word, 3, SimTime::ZERO)
            .unwrap();
        machine
            .bus

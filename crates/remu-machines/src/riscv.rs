...
use remu_devices::{
    EspAnalogI2c, EspGpio, EspLedc, EspLedcHandle, EspSpiMem, EspTimerGroup, EspTimerGroupHandle,
    ...
    pub fn new(target: TargetId) -> Result<Self, MachineError> {
        ...
        match target {
            ...
            TargetId::Esp32c6 => {
                ...
                let (ledc, handle) =
                    EspLedc::new("esp32c6.ledc", "board.esp32c6.ledc", signals.clone())?;
                bus.map_device("esp32c6.ledc", 0x6000_7000, 0x1000, Box::new(ledc))?;
                esp_ledc = Some(handle);
    ...
    pub fn run_with_stimuli(
        ...
        let reason = loop {
            ...
            self.now = self
                ...
                .map_err(|_| MachineError::TimeOverflow)?;
            stats.time = self.now;
            if let Some(ledc) = &self.esp_ledc {
                stats.events = stats.events.saturating_add(u64::from(ledc.poll(self.now)?));
            }

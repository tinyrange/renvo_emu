...
fn target_xml(target: &impl DebugTarget) -> String {
    ...
    let mut xml = format!(
        "<?xml version=\"1.0\"?><target><architecture>{}</architecture><feature name=\"org.remu.core\">",
...
mod tests {
    use super::*;
    use remu_core::RegisterValue;

    #[test]
    fn register_packet_is_little_endian_and_includes_pc() {
        let snapshot = CpuSnapshot {
            architecture: remu_core::Architecture::RiscV32,
            pc: 0x1234_5678,

...
| ESP32-S3 | Xtensa LX7 windowed compiler subset | DRAM, IRAM, 16 MiB IROM and DROM windows | Windowed ABI/exception/atomic/FPU qualification; GPIO matrix low bank waveform, native-address UART0 FIFO transcript, and eFuse OTP/status slice |
...
The ESP32-S3 eFuse slice retains the native staging, read-data, command,
status, and interrupt registers. It provides deterministic factory identity
words, read-only OTP views, one-way bitwise programming for the documented
blocks, key-block read redaction through `RD_DIS`, and read/program completion
interrupts. It intentionally does not model programming voltage, Reed-Solomon
...
- Espressif’s official [ESP32-S3 eFuse register definitions](https://github.com/espressif/esp-idf/blob/master/components/soc/esp32s3/register/soc/efuse_reg.h)

...
master command-link subset. Native `CTR.TRANS_START`, restart/write/read/stop/
end command entries, FIFO transmit/receive bytes, transfer-completion raw/
enable/status/clear bits, and deterministic host injection are covered.
Command-link timing, arbitration, clock stretching, DMA, LP-I2C, and the
interrupt matrix remain outside this slice.

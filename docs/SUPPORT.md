...
| RP2350 | Cortex-M33 Thumb subset or Hazard3 RV32IMAC/B subset | 16 MiB XIP window, 520 KiB SRAM | SIO GPIO, UART0, TIMER interrupt, PIO0 waveform, and ACCESSCTRL policy proofs in both CPU modes |
...
Both RP2350 CPU modes map the security-permission block at `0x40060000`. The
model implements the documented reset masks for ROM, XIP, SRAM banks, DMA,
USB, PIO, timers, UARTs, OTP, clocks, and other APB blocks, GPIO non-secure
masks, `FORCE_CORE_NS`, atomic set/clear/XOR aliases, configuration reset, and
write-once lock state (including the permanently locked DMA bit).

The current CPU contract does not carry secure/non-secure or privilege tags,
so ACCESSCTRL is an observable policy model rather than an enforced bus filter.
The permission values and GPIO masks are available to higher-level machine
composition and security tests; bus-fault attribution and TrustZone state

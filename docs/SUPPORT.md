...
`0x4005e000`. `DADR0`, `DACR.DAOE0`, `DADPR.DPSEL`, synchronous-start, and
VREF control registers are modeled, with a 12-bit `board.r7fa4m1ab3cfm.dac0`
signal for traces and host assertions. The model reports a deterministic
digital sample; analog amplifier settling and pin mux electrical behavior are
outside this functional boundary. Register choices follow the [RA4M1 hardware

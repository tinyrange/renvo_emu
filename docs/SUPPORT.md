...
| ESP32-C6 | RV32IMAC/Zicsr machine/user subset | ROM, HP/LP SRAM, 16 MiB IROM window | GPIO matrix pin 2 waveform, native UART0 transcript, two-channel RMT symbol waveform, user traps, and PMP CSR visibility |
...
ESP32-C6 RMT model covers the two native APB FIFO transmitter channels and
VCD-visible channel outputs; native channels 2/3 are receive-only and remain
raw register windows. Receiver DMA, carrier modulation, interrupt timing, and
GPIO-matrix routing remain unsupported.

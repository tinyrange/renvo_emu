...
functional slice implements the BCD calendar counters, four-digit year,
calendar alarm enables, `RCR1`/`RCR2`/`RCR4` control and the RTC alarm ELC
event (`0x026`). One abstract simulation tick advances one RTC second; this
is deterministic functional behavior, not a 32.768-kHz clock model. The
register layout follows the [RA4M1 hardware manual](https://www.renesas.com/en/document/mah/renesas-ra4m1-group-users-manual-hardware).

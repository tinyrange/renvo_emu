...
The CH32V003 and CH32V006 models now expose the native AFIO remap window at
`0x4001_0000` and EXTI block at `0x4001_0400`. AFIO `PCFR1` is retained and
`EXTICR` selects PA, PC, or PD sources for EXTI0--7. The functional EXTI model
implements rising/falling edge detection, software-triggered flags,
write-one-to-clear status, and PFIC delivery on the EXTI7_0 interrupt line.
External GPIO stimuli are sampled on the deterministic simulation timeline.

This slice does not model analog/PVD or auto-wakeup sources, event-only wake
semantics, alternate-function electrical muxing, or undocumented remap side
effects. Register addresses and field meanings follow the
[CH32V003 reference manual](https://ch32-riscv-ug.github.io/CH32V003/datasheet_en/CH32V003RM.PDF).

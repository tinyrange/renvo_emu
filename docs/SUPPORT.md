...
| ESP32-C6 | RV32IMAC/Zicsr machine/user subset | ROM, HP/LP SRAM, 16 MiB IROM window | GPIO matrix pin 2 waveform, native UART0 transcript, LP UART TX/RX, user traps, and PMP CSR visibility |
...
ESP32-C6 LP UART is mapped at `0x600b1400` with its 16-byte RX/TX FIFO,
interrupt raw/status/enable/clear registers, clock divider, framing thresholds,
status counts, reset controls, version, and ID registers. Host tests can queue
RX bytes and collect TX bytes; firmware-visible TX/RX values are emitted as
`board.esp32c6.lp_uart.tx` and `.rx` VCD signals. Low-power wake sequencing,
electrical pin muxing, and exact serial timing remain unsupported.

...
| ESP32-C6 | RV32IMAC/Zicsr machine/user subset | ROM, HP/LP SRAM, 16 MiB IROM window | GPIO matrix pin 2 waveform, native UART0 transcript, six-channel LEDC PWM waveform, user traps, and PMP CSR visibility |
...
ADC/serial buses, watchdog resets, or virtual ESP radio connectivity. The
ESP32-C6 LEDC model is a native register/trace slice; it does not yet route
LEDC outputs through the ESP GPIO matrix or implement fade/gamma/event-task
and overflow-interrupt behavior.

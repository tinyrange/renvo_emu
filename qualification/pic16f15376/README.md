...
lowering, generated arithmetic helpers, GPIO, Timer0 interrupt entry/return,
DAC1 code selection, and EUSART1 output. Every build must emit
`PIC16F15376:OK\nIRQ\n`.
...
ANSEL A–E, PPS register storage, PIR/PIE routing, Timer0/1, the normalized
DAC1 code and enable/output controls, EUSART1 transmit, and watchdog reset.
Timer and serial timing are deterministic approximations; DAC output is a
digital code oracle rather than an analog voltage simulation.
...
and runs the original register-level Timer0 and DAC1 programs described under
[`fixture`](fixture/README.md). They are written from the public PIC16F15376 data
sheet and must produce the expected timer edge and DAC enable transition
without incorporating Microchip application-note or SDK source.

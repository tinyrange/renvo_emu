...
explicit ICU routing, GPT0/GPT1 interrupt entry and SCI9 UART (`RA4M1\n`).

Implemented functionally are SYSTEM clock/oscillator and module-stop state,
IOPORT, ICU, GPT0 and GPT1, SCI9 and watchdog. GPT1 is mapped at `0x40078100`;
its counter/overflow slice uses ELC event `0x065` and can be routed through any
configured ICU IELSR slot. USB, CAN, analog/LCD blocks, exact clock timing and
the UNO R4 Wi-Fi-board bridge are unsupported. VCD records the

...
__attribute__((noreturn, section(".text.start"))) void _start(void)
{
    ...
    if (*(volatile uint32_t *)UHCI_DATE_REG(0) != 0x02010090u ||
        *(volatile uint32_t *)UHCI_DATE_REG(1) != 0x02010090u) {
        *exit_code = 1;
        ...
        *(volatile uint32_t *)UHCI_APP_INT_SET_REG(0) = UHCI_APP_CTRL0_INT_SET;
        if ((*(volatile uint32_t *)UHCI_INT_ST_REG(0) & UHCI_APP_CTRL0_INT_ST) == 0) {
            ...
            *(volatile uint32_t *)UHCI_INT_CLR_REG(0) = UHCI_APP_CTRL0_INT_CLR;
            if (*(volatile uint32_t *)UHCI_INT_ST_REG(0) != 0) {
                *exit_code = 4;
            } else {
                *(volatile uint32_t *)UHCI_INT_ENA_REG(1) = UHCI_APP_CTRL1_INT_ENA;
                *(volatile uint32_t *)UHCI_APP_INT_SET_REG(1) = UHCI_APP_CTRL1_INT_SET;
                if ((*(volatile uint32_t *)UHCI_INT_ST_REG(1) & UHCI_APP_CTRL1_INT_ST) == 0) {
                    *exit_code = 5;
                } else {
                    *(volatile uint32_t *)UHCI_INT_CLR_REG(1) = UHCI_APP_CTRL1_INT_CLR;
                    *exit_code = (*(volatile uint32_t *)UHCI_INT_ST_REG(1) == 0) ? 0 : 6;
                }
            }
        }
    }

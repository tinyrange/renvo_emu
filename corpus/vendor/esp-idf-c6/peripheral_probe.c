...
int main(void)
{
    ...
    CHECK(EXTMEM_DATE_REG, 0x0fffffffu, 0x02202080u, 34);
    CHECK(PMU_DATE_REG, 0x7fffffffu, 35676752u, 35);
    CHECK(LP_AON_DATE_REG, 0x7fffffffu, 35672704u, 36);
    CHECK(LP_TIMER_DATE_REG, 0x7fffffffu, 34672976u, 37);

    WRITE32(PLIC_MXINT_ENABLE_REG, 1u << 6);
    WRITE32(PLIC_MXINT6_PRI_REG, 7u);
    WRITE32(PLIC_MXINT_THRESH_REG, 3u);
    CHECK(PLIC_MXINT_ENABLE_REG, 1u << 6, 1u << 6, 38);
    CHECK(PLIC_MXINT6_PRI_REG, 0xfu, 7u, 39);
    CHECK(PLIC_MXINT_THRESH_REG, 0xffu, 3u, 40);
    WRITE32(PLIC_UXINT_ENABLE_REG, 1u << 4);
    WRITE32(PLIC_UXINT4_PRI_REG, 5u);
    CHECK(PLIC_UXINT_ENABLE_REG, 1u << 4, 1u << 4, 41);
    CHECK(PLIC_UXINT4_PRI_REG, 0xfu, 5u, 42);

    WRITE32(CLINT_MINT_MTIME_L_REG, 0u);
    WRITE32(CLINT_MINT_MTIMECMP_L_REG, 1u);
    WRITE32(CLINT_MINT_TIMECTL_REG, 3u);
    CHECK(CLINT_MINT_TIMECTL_REG, 7u, 7u, 43);
    WRITE32(CLINT_UINT_SIP_REG, 1u);
    CHECK(CLINT_UINT_SIP_REG, 1u, 1u, 44);

    WRITE32(EXTMEM_L1_CACHE_SYNC_ADDR_REG, 0x42800000u);
    WRITE32(EXTMEM_L1_CACHE_SYNC_SIZE_REG, 64u);
    WRITE32(EXTMEM_L1_CACHE_SYNC_CTRL_REG, EXTMEM_L1_CACHE_INVALIDATE_ENA);
    CHECK(EXTMEM_L1_CACHE_SYNC_CTRL_REG, 1u << 4, 1u << 4, 45);
    WRITE32(PMU_LP_CPU_PWR1_REG, 1u << 4);
    CHECK(PMU_LP_CPU_PWR1_REG, 0xffffu, 1u << 4, 46);
    WRITE32(LP_AON_STORE0_REG, 0x13579bdfu);
    CHECK(LP_AON_STORE0_REG, UINT32_MAX, 0x13579bdfu, 47);
    WRITE32(LP_TIMER_TAR0_LOW_REG, 0x1234u);
    CHECK(LP_TIMER_TAR0_LOW_REG, UINT32_MAX, 0x1234u, 48);

    uint32_t pmpaddr15;
    __asm__ volatile ("csrw pmpaddr15, %1; csrr %0, pmpaddr15"
                      : "=r"(pmpaddr15) : "r"(0x12345u));
    if (pmpaddr15 != 0x12345u) {
        return 49;
    }
    return 0;
}

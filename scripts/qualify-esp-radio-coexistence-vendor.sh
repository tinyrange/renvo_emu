...
requirements=qualification/radio/coexistence-vendor-requirements.json
artifact_root=${REMU_RADIO_COEX_ROOT:-.remu/qualification/radio-coexistence-vendor}
...
sdkconfig_defaults=$(jq -r '.sdkconfig_defaults' "$requirements")
...
expected_host_frames=$(jq -r --arg chip "$chip" '.chips[$chip].expected_host_frames' "$requirements")
...
docker run --rm \
    ...
    "$idf_image" \
    bash -lc "IDF_TARGET=$chip SDKCONFIG_DEFAULTS=/workspace/$sdkconfig_defaults idf.py -C /workspace/$firmware_project -B /out/build -D SDKCONFIG=/out/sdkconfig build && cd /out/build && python -m esptool --chip $chip merge-bin -o /out/$chip-coexistence-flash.bin @flash_args" \
...
run_vendor_coexistence "$chip_root/result.json" "$chip_root/radio-replay.json" \
...
do
    ...
    then
        echo "$chip vendor coexistence milestone missing from UART: $required_uart" >&2
        exit 1
    fi
done

run_vendor_coexistence "$chip_root/result-repeat.json" "$chip_root/radio-replay-repeat.json" \
